...
const en: Locale = {
    ...
    apiFormatDesc: "Protocol AI curation uses to call the LLM. Ollama for Ollama; OpenAI-compatible for llama.cpp, LM Studio, MLX, vLLM, OpenAI, etc.",
    ...
    noticeScopeDefaultChanged: "Vault Curate: Search now includes forgotten (Cold) notes by default. Yours is still set to Hot only; switch it with the scope buttons under the search box, or in Settings → Advanced.",
...
const zhTW: Locale = {
    ollamaUrl: "Embedding 伺服器",
    ollamaUrlDesc: "Embedding 伺服器位址",
    apiFormat: "API 格式",
    apiFormatDesc: "AI 整理呼叫 LLM 使用的協定。Ollama 用於 Ollama；OpenAI-compatible 用於 llama.cpp、LM Studio、MLX、vLLM、OpenAI 等",
    apiFormatOllama: "Ollama",
    apiFormatOpenAI: "OpenAI-compatible",
    ...
    noticeScopeDefaultChanged: "Vault Curate：搜尋現在預設會包含很久沒碰的冷門（Cold）筆記。你的設定目前仍是只搜 Hot，可用搜尋框下方的範圍按鈕切換，或到「設定 → 進階」調整。",
    languageLabel: "繁體中文",
...
const zhCN: Locale = {
    ollamaUrl: "Embedding 服务器",
    ollamaUrlDesc: "Embedding 服务器地址",
    apiFormat: "API 格式",
    apiFormatDesc: "AI 整理调用 LLM 使用的协议。Ollama 用于 Ollama；OpenAI-compatible 用于 llama.cpp、LM Studio、MLX、vLLM、OpenAI 等",
    apiFormatOllama: "Ollama",
    apiFormatOpenAI: "OpenAI-compatible",
    embeddingModel: "Embedding 模型",
    ...
    noticeScopeDefaultChanged: "Vault Curate：搜索现在默认会包含很久没碰的冷门（Cold）笔记。你的设置目前仍是只搜 Hot，可用搜索框下方的范围按钮切换，或到「设置 → 高级」调整。",
    languageLabel: "简体中文",
