...
### Changed
- **Settings are grouped by what they actually control.** The first section is now called **Embedding & indexing** (it was "Quick setup") and its URL field says **Embedding server** — no more guessing which server that generic "Server URL" meant. **API format** moved into the AI curation section: it only ever governed how the plugin talks to the LLM, never embedding, so it now lives next to the things it affects (and mobile, which has no LLM features, no longer shows it at all). The two model dropdowns each ask the right server in the right protocol for their list, and the setup wizard button sits at the end of the AI curation section, still available with curation switched off.
...
- **AI curation no longer refuses a working OpenAI-compatible server.** Before starting, curation pinged the server's root URL and demanded a 200 — which Ollama happens to answer, but servers like `mlx_lm.server` answer 404 there while being perfectly reachable at `/v1/*`. You'd see "Cannot connect" against a server that was up. The pre-flight now speaks whichever protocol you configured, using the same probe the settings page already uses — so the settings page and the actual run can no longer disagree about the same server.
- **Tags returned as one string are no longer dropped.** Some models answer with `"tags": "a b c"` instead of a JSON array (measured: Qwen3.5-4B, 4 notes out of 15). Those tags used to vanish entirely; they are now split on spaces and commas and go through the exact same cleanup as tags that arrive as an array.
- **A reply the plugin couldn't parse can no longer smuggle invisible characters into a note.** When a model answered with something that wasn't valid JSON (a refusal, a chatty aside, malformed output), the last-resort path wrote that text into the note's `description` without the control-character and invisible-character scrubbing every other path applies. Now all three paths scrub identically, so ANSI escapes, NUL/carriage-return, and zero-width/tag code points can't reach your frontmatter that way.
- **The server-address guard now also covers IPv4-mapped IPv6.** The check that refuses to point a server field at cloud-metadata / link-local addresses (e.g. `169.254.169.254`) missed the IPv6-mapped spelling of the same address (`[::ffff:169.254.169.254]`); it now decodes and blocks that form too. Both the embedding server and the new AI curation server field run through the same guard.
