...
- **尋找相似筆記**：對任一 `.md` 右鍵選 **VC: 尋找相似筆記**，結果進側邊欄、可直接拖到 Canvas。相似度不會被模板騙走：比對時會剝除 markdown 結構符號、並把筆記屬性裡的 `description` 摘要納入排序，讓人物卡找到*那個人*的對話，而不是九張同模板的兄弟卡。繁體筆記庫底層會先轉成簡體再理解語意（你看到的儲存文字、搜尋結果與摘要全程保持繁體），排名更準。
