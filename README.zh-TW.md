...
中文筆記的語意搜尋與關聯整理。語意搜尋 · 關聯圖 · 語意路徑 · Hot/Cold 遺忘再發現 · 桌機建索引、手機直接搜 · 預設不上雲
...
- 大致記得寫過，但沒用到當初那個關鍵字，就是找不到
- 好幾篇筆記其實在講相關的事，卻散落各處，要一篇篇手動建連結很麻煩
- 多年累積的筆記像逝去的記憶，寫完就再也沒打開過
- 不想讓 AI 把所有筆記無腦整理成一套 Wiki：那是 AI 的記憶庫，不是我的

Vault Curate 就是為這幾件事做的。

### 幾個堅持

- **預設不上雲**：預設全部在自己電腦上算，模型約 110 MB、只下載一次，不用 API key。AI 整理要接雲端服務才會送出資料，決定權在你自己手上。
- **中文友善**：內建的中文模型對人名、專有名詞、口語詞特別準；繁體筆記也照樣好用。其他語言可以改接 Ollama 或 OpenAI 相容服務。
- **不替你思考**：不在背景偷跑 AI，不自動改你的筆記。每一條關聯都只是建議，要或不要由你決定。
- **手機也能用**：桌機建好索引，跟著 iCloud、Obsidian Sync 或 Syncthing 同步過去，手機直接搜，不用重算、不用下載模型。

---

## 功能一覽

三件事零設定、裝好就能用；第四件（AI 整理）預設關閉，要你自己打開。

1. **找到**：語意搜尋，換句話說也找得到，還會顯示命中的那一段。
2. **看見關聯**：把內容相關、但你沒連過的筆記畫成 Canvas 關聯圖。
3. **不遺忘**：很久沒動的舊筆記會重新浮到你眼前。
4. **AI 整理**（選配）：幫筆記寫摘要和標籤，把結果分群做成主題目錄。

**每一條關聯都等你決定。**看順眼的，一鍵變成真的連結（wikilink）；不要的，按 **✕**，這對筆記就不會再被推薦，改名、重建索引也不會回來。Hot/Cold 也照你的動作走：*編輯*一篇筆記才算你又用到它，只是打開看看不算。
...
- **看得到命中哪一段**：搜尋結果會顯示命中的段落，搜尋的字會標亮；點下去直接打開到那一段。在搜尋彈窗裡按 **Alt+Enter**，會在游標處插入該篇的連結，而不是打開它。
- **Hot／Cold／全部**：搜尋預設涵蓋所有筆記，包括很久沒碰的。搜尋框下方的按鈕可以只搜最近碰過的（Hot）或很久沒碰的（Cold）。
- **長筆記整篇都讀**：內建模型會讀完長筆記的每一段（單篇上限六萬字），不只開頭，所以藏在筆記深處的段落也能靠語意找到。
...
- **你自己的同義詞**：「進階 → 同義詞表」可以教搜尋你的私有詞彙，像人名暱稱、機構簡稱、模型不可能知道的內部用語（例如 `小陳 = 陳大文`）。查詢裡出現其中一種寫法，另一種也會一併搜到。手機的關鍵字模式尤其受益。
- **結果導出 Canvas**：搜尋分頁的 **結果導出 Canvas** 按鈕把目前的結果攤到一張可編輯的 Canvas 上，查詢在正中間，前 12 篇圍成一圈，每條邊標著該篇的相關度。這張圖是「一次搜尋的結果空間」，下面的關聯圖則是「一篇筆記的鄰域」。結果超過 12 篇時會有通知說明少放了幾篇，不會無聲截斷。
...
- **灰邊**（帶箭頭）：已有 wikilink
- **青框**：Cold 筆記
- **綠邊**：與搜尋查詢的相關度。它只出現在從搜尋分頁導出的結果 Canvas 上，不用來表示兩篇筆記之間的關係
...
- 也能導成 Canvas：搜尋分頁的 **結果導出 Canvas** 按鈕對查詢結果做同一件事，讓「一次搜尋的結果空間」和「一篇筆記的鄰域」可以擺在一起看
...
AI 模型在「**設定 → AI 整理**」獨立指定（本機 Ollama，或任何 OpenAI 相容服務）。「**AI 輸出語言**」可以指定 AI 寫出來的語言，不必跟著 Obsidian 介面：跟隨介面（預設）、English、繁體中文、简体中文，或自己填其他語言名稱。介面用英文、筆記用中文的人特別用得到。
...
> 在 Obsidian 關閉期間被刪掉的筆記（透過同步工具、git 或檔案管理員），下次啟動時會自動從索引移除，搜尋不會再給你一篇已經不存在的筆記。
...
| **AI 整理** | 啟用開關；LLM 提供者；LLM 模型；AI 輸出語言 | 關；Ollama；qwen3:1.7b；跟隨介面 |
| **進階** | 結果數量、最低分數、關聯圖資料夾、相關筆記小節標題、雙向寫入、已隱藏的建議（計數加管理/恢復）、Hot 期間、搜尋範圍、分段大小（僅 Ollama／OpenAI 相容端點）、同義詞表、自動索引、重建加更新、索引統計 | 見面板 |

切換語意引擎（Embedding 提供者）或模型會跳確認視窗，索引清空重建。

### 疑難排解

- **在編輯模式下點表格裡的結果，不會捲到那一列**：Obsidian 內建搜尋也有同樣的限制（編輯時表格是整塊顯示）。把筆記切到閱讀模式，點下去就會停在那一列。
