...
**語意路徑（Canvas）**：選任意兩篇筆記，找出連接它們的**中繼筆記鏈**。它在語意 k-NN 圖上跑瓶頸路徑搜尋，整條鏈以*最弱的一跳*評分，牽強的環節藏不進強環節裡。跑「**生成語意路徑（Canvas）**」再挑終點；若不存在全程夠強的鏈，會誠實告知「不連通」並附實際數字，這是資訊，不是錯誤。底層圖按需建（2.5k 筆記約 4 秒），索引沒變就重複使用、再查免等。
...
好筆記不該因為你忘了它就等於不存在。筆記依**內部連結加近期活躍度**自動分級：**Hot**（有被連結或最近碰過）、**Cold**（孤立或一段時間沒動），界線可在「進階 → Hot 期間（天）」調。

Discover 作用在**筆記**上而非查詢字串，主動浮現你最近沒碰、但語意相關的 Cold 筆記：

- **當前筆記**：打開某篇時自動出現相關筆記，Cold 視覺標示（「你還沒讀過這篇」）
- **全域**：與整個 Hot 池最相關的 Cold 筆記，這是刻意的盲點挖掘
...
| `發掘相關的 Cold 筆記` | 全域發掘：與 Hot 池最相關的 Cold 筆記 | 永遠可用 |
...
筆記右鍵選單：**VC: 尋找相似筆記**、**VC: 生成關聯圖**、**VC: 生成語意路徑**、**VC: 在此圖展開**（canvas 開啟時）、**VC: 生成 description**（需啟用 AI 整理）。
...
| **進階** | top results、最低分數、關聯圖資料夾、Hot 期間、搜尋範圍、chunk 大小加 overlap、同義詞表、自動索引、重建加更新、索引統計 | 見面板 |
